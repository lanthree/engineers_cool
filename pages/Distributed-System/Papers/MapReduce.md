...
原论文介绍就不翻译了，本文是从[这里](https://pdos.csail.mit.edu/6.824/papers/mapreduce.pdf)阅读原文时，为了更好的记忆笑话，而做的个人翻译。
...
此外，R总会被用户限制，因为每个reduce任务会产出一个独立的输出文件。在实践中，我们倾向于选择合适的M使得每个独立的map任务的输入数据大小在16MB～64MB之间（使得上面描述的本地化优化能有最大性能），我们一般令R为机器数量的小几倍。我们总是倾向于使用如下配置完成MapReduce计算：M=20w，R=5000，2000台worker机器。
